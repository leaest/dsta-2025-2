...
Check if the container is running with `docker ps` --> `STATUS` --> "Up ... seconds" or check the container status through the Docker desktop application.
...
**PostgreSQL**
- The db service runs a PostgreSQL server using the official postgres:18.1 image.
